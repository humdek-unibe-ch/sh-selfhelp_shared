...
- Should README examples using `THEME_TOKENS` be updated, or should a `THEME_TOKENS` aggregate export be added?
- Should an ESLint v9 flat config be added so `npm run lint` works?
- Should baseline Vitest tests be added so `npm run test` is a useful gate?
- Should `license-check-and-add` config be fixed, or should README/CHANGELOG receive headers?
- Should `CHANGELOG.md` include the current `1.0.1` release?
- Is `file:` pass-through in `resolveAssetUrl()` intended for both web and mobile consumers?